...
const { readReviewSheet } = require('./reviewRoundTrip');
...
// -----------------------------------------------------------------------
// 3. Pass rates (from Attempt data)
// -----------------------------------------------------------------------
async function computePassRates(examId) {
  ...
  // Per-domain across all attempts
  const domainAll = {};
  attempts.forEach((a) => {
  ...
  // Per-question item analysis (p-value = proportion correct, discrimination)
  const questionStats = {};
  attempts.forEach((a) => {
  ...
  // For each question: correlate "got it right (1/0)" with total score across attempts
  const attemptScores = {};
  attempts.forEach((a, ai) => { if (a.score != null) attemptScores[ai] = a.score; });
  ...
  for (const [qid, pairs] of Object.entries(attemptQMap)) {
    ...
    const pValue = sx / n;
    discrimination.push({ qid, rpb, pValue, n });
  }
  ...
  for (const [qid, opts] of Object.entries(questionOptionMap)) {
    ...
    if (total < 5) continue;
    const allOpts = Object.keys(opts);
  ...
  // Cronbach's alpha (on binary correct/incorrect per item)
  ...
  if (attempts.length >= 10) {
    ...
    if (validAttempts.length >= 10 && itemList.length >= 5) {
      ...
      if (totalVar > 0) {
        cronbachAlpha = (k / (k - 1)) * (1 - sumItemVar / totalVar);
      }
      alphaItemCount = k;
      alphaAttemptCount = n;
    }
  ...
  if (scores.length >= 5) {
    ...
    distribution = { n, mean: mean.toFixed(1), sd: sd.toFixed(1), skew: skew.toFixed(2), bins };
  }

  return {
    totalAttempts: attempts.length,
    uniqueCases: Object.keys(byCase).length,
    scores: { mean: mean.toFixed(1), median: median.toFixed(1), min: sorted[0], max: sorted[sorted.length - 1] },
    passRate: scores.length ? ((passing / scores.length) * 100).toFixed(1) : 'N/A',
...
// -----------------------------------------------------------------------
// 4. Interrater reliability (from ReviewBatch sheets + user responses)
// -----------------------------------------------------------------------

// Cohen's kappa for two raters' binary decisions
...
async function computeIrr(examId) {
  const result = { reviewerAgreement: null, userResponseConsistency: null };

  // --- Reviewer agreement from ReviewBatch sheets ---
      ...
      for (let i = 0; i < reviewerDecisions.length; i++) {
        for (let j = i + 1; j < reviewerDecisions.length; j++) {
          ...
          if (commonKeys.length >= 5) {
            ...
            const kappa = cohensKappa(va, vb);
            const pct = pctAgreement(va, vb);
            pairResults.push({ raterA: a.name, raterB: b.name, n: commonKeys.length, kappa, pctAgree: pct });
          }
        }
      }
  ...
  // --- User response consistency (same user, same question, different attempts) ---
  const attempts = await Attempt.find({ examId, completedAt: { $ne: null } })
  ...
  if (retestPairs.length >= 10) {
    ...
    result.userResponseConsistency = {
      ...
      consistent,
      rate: (consistent / retestPairs.length * 100).toFixed(1),
    };
  }

  // --- Inter-user agreement on the same question (proportion choosing the same option) ---
  ...
  if (interUser.length >= 10) {
    ...
    result.interUserAgreement = {
      ...
      avgModalPct: (avgModal * 100).toFixed(1),
