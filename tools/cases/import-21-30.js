#!/usr/bin/env node
// Import cases 21-30 into MongoDB (passreadyprep.contents collection)
// Usage: MONGO_URI=mongodb+srv://... node tools/cases/import-21-30.js
...
async function run() {
  ...
  for (const c of CASES_21_30) {
    const exists = await Content.findOne({ externalId: c.id });
    if (exists) { console.log('  SKIP ' + c.id + ' (already exists)'); skipped++; continue; }
    ...
    await Content.create({
      ...
      status: 'sme_review', // needs review before publishing
      caseSim: c,
    });
    console.log('  ADD  ' + c.id + ': ' + c.title);
    added++;
  }
