...
const SELF_DISQUALIFYING_RE = /\b(but (does|is) not|suggestive but|does not establish|not the (feature|finding|priority)|on its own|by itself|without other (diagnostic )?features|taken as|treated as evidence|entirely|completely)\b/i;
...
function mergeLengthFix(candidate, replyOptions) {
  ...
  for (const o of got) {
    ...
    if (SELF_DISQUALIFYING_RE.test(t)) return { ok: false, reason: 'self-disqualifying', detail: 'id ' + o.id + ': forbidden pattern in "' + t.slice(0, 80) + '"' };
