...
- **Paid / logged-in** users: all published cases under the `ncmhce` exam.
- **Free / anonymous** users: capped at `FREE_CASE_LIMIT` in
  `routes/content.js` (list is sliced, and per-case access is gated to the first
  N by `category, externalId`). If the app "only shows a few," that cap — not a
  data problem — is usually why. Adjust the constant to change the free preview.
