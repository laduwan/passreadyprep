...
- OPTION BALANCE (critical — items are rejected if the key is detectable by surface cues): write all four options to the SAME visual weight. Aim for each option in the 12-22 word range and within ~20% of each other in length, with parallel grammar (e.g., all phrased as the action the clinician takes) and equal specificity. Every distractor must be a fully formed, genuinely tempting action a partially-prepared clinician might pick — never a short throwaway. The keyed answer must NOT be the longest, most detailed, most clinically "complete," or most hedged option; if it is, either trim the key or build the distractors up to match BEFORE finalizing. Each distractor must be wrong for a specific, nameable reason stated in its rationale. Vary which option (a/b/c/d) is correct from question to question; never default the answer to the first option.
...
- SELF-CHECK before output: for every question verify (1) exactly one option has isCorrect=true and weight 3, the others weight 0/-1/-2; (2) the correct option is NOT the longest of the four; (3) all four options read as parallel, plausible clinical actions of similar length. Rewrite any question that fails any check before you produce the JSON.
